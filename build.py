...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--with-fdk-aac", action="store_true")
...
def main() -> None:
    ...
    if not args.target.startswith("macos-") and os.environ.get("FFMPEG_MT_IN_CONTAINER") != "1":
        run_in_container(root, args.target, args.jobs, args.with_fdk_aac, args.clean)
        return
    ...
    build(
        ...
        jobs=args.jobs,
        with_fdk_aac=args.with_fdk_aac,
        clean=args.clean,
    )
